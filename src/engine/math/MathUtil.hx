...
class MathUtil {
	/**
	 * It is recommended to use this to get `PI` instead of using `Math.PI`.
	 * 
	 * Why? Because `Math.PI` will likely conflict with Raylib (for no reason)
	 */
	 ...
	 */
	public static function pointInRectangle(pointX:Float, pointY:Float, rect:Point4D):Bool {
	...
	/**
	 * Check if the distance between two Sprites is within a specified number.
	...
	/**
	 * Find the distance (in pixels, rounded) from an Sprite
	 * to the given Point2D, taking the source origin into account.
	 *
	...
	/**
	 * Check if the distance from an Sprite to the given
	 * Point2D is within a specified number.
