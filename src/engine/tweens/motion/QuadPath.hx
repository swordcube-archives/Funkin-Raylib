...
import engine.tweens.Tween.TweenManager;
import engine.tweens.Tween.TweenOptions;
...
class QuadPath extends Motion {
	 ...
	 */
	public function getPoint(index:Int = 0):Point2D {
		if (_points.length == 0) {
	...
	function getCurveLength(start:Point2D, control:Point2D, finish:Point2D):Float {
		var p1 = new Point2D();
		var p2 = new Point2D();
