...
class PlayState extends MusicBeatScene {
	...
	public var keyBinds:Array<Int> = [Keys.S, Keys.D, Keys.K, Keys.L];
	...
	public function keyShit() {
		...
		notes.forEachAlive((note:Note) -> {
			if (!note.mustPress || !note.canBeHit || note.isSustainNote)
				return;
		...
		for (note in possibleNotes) {
			if (!Game.keys.justPressed(keyBinds[note.noteData]) || hitBlocked[note.noteData])
				continue;
	...
	override function beatHit(curBeat:Int) {
		...
		#if linux
		var resyncTime:Float = 30 * Game.timeScale;
		#else
	...
	public function resyncVocals() {
		Logs.trace('Resynced vocals at song position: ${Conductor.position} with an inst and vocals times of: ${inst.time}, ${vocals.time}', INFO);
