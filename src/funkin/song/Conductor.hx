...
class Conductor {
	...
	public static var onBeatHit:TypedSignal<Int->Void> = new TypedSignal<Int->Void>();
	public static var onStepHit:TypedSignal<Int->Void> = new TypedSignal<Int->Void>();
	public static var onSectionHit:TypedSignal<Int->Void> = new TypedSignal<Int->Void>();
	...
	static function stepHit() {
		if (PlayState.SONG != null && Game.scene == PlayState.current) {
			if (oldStep < curStep)
				updateSection();
			else
				rollbackSection();
