...
import funkin.objects.FNFSprite.AnimationContext;
import engine.math.Point2D;
import funkin.objects.TrackingSprite;
...
class HealthIcon extends TrackingSprite {
    ...
    */
    public function loadIcon(char:String) {
        this.char = char;
        if(!FileSystem.exists(Paths.image('gameplay/icons/$char'))) {
            char = "face";
            this.char = char;
        }
        loadGraphic(Paths.image('gameplay/icons/$char')); // have to load stupidly first to get the icon size

        jsonData = {};
        var jsonPath:String = Paths.json('images/gameplay/icons/$char');
        try {
        ...
        if(jsonData.animations.length <= 0)
		    loadGraphic(Paths.image('gameplay/icons/$char'), Std.int(width / jsonData.icons), jsonData.iconHeight);
        else {
            frames = Paths.getSparrowAtlas('gameplay/icons/$char');
            for(anim in jsonData.animations) {
                anim.setFieldDefault("fps", 24);
                anim.setFieldDefault("loop", true);
                if(anim.indices != null && anim.indices.length > 0)
                    addAnimByIndices(anim.animName, anim.spritesheetAnim, anim.indices, anim.fps, anim.loop, new Point2D(anim.offsets.x, anim.offsets.y));
                else
                    addAnim(anim.animName, anim.spritesheetAnim, anim.fps, anim.loop, new Point2D(anim.offsets.x, anim.offsets.y));
            }
        }
    ...
    */
    ...
    override function update(elapsed:Float) {
        ...
        if(Game.scene != PlayState.current) return; 
    ...
    override function playAnim(name:String, force:Bool = false, ?context:AnimationContext = NORMAL, frame:Int = 0) {
        ...
        offset.add(jsonData.positionOffset.x, jsonData.positionOffset.y);
    }
}