...
class Paths {
	public static final scriptExtensions:Array<String> = ["hx", "hxs", "hsc", "hscript"];
	...
	public static function songInst(song:String, ?diff:String = "normal") {
		var songPaths:Array<String> = [asset('songs/${song.toLowerCase()}/Inst-$diff.ogg')];
		for (p in songPaths) {
	...
	public static function songVoices(song:String, ?diff:String = "normal") {
		var songPaths:Array<String> = [asset('songs/${song.toLowerCase()}/Voices-$diff.ogg')];
		for (p in songPaths) {
			if (FileSystem.exists(p))
				return p;
		}
